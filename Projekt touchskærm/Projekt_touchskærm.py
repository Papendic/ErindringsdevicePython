...
class RelativeWindow:
	...
	ButtonList=[]
	_id=0
	def openRelativeWindow(iid):
		cwd = os.getcwd()
		ID=iid-1
		RelativeWindow._id=iid
		...
		PlaySound = Button(newWindow, height ='140', width='140', bg = 'LightPink1', fg='white', image=Pictures.NewPlaybuttonBillede, command=lambda: AudioPlayer.StopOrPlay(RelativeWindow._id,RelativeWindow.isPlaying))
		PlaySound.pack()
...
class MainWindow:
	...
	isPlaying=False
	if(len(relativeObj) > 0 and relativeObj[0].PersonImage!=None):
	...
	if(len(relativeObj) > 1 and relativeObj[1].PersonImage!=None):
	...
	if(len(relativeObj) > 2 and relativeObj[2].PersonImage!=None):
	...
	if(len(relativeObj) > 3 and relativeObj[3].PersonImage!=None):
	...
	if(len(relativeObj) > 4 and relativeObj[4].PersonImage!=None):
	...
	if(len(relativeObj) > 5 and relativeObj[5].PersonImage!=None):
	...
	if(len(relativeObj) > 6 and relativeObj[6].PersonImage!=None):
	...
	if(len(relativeObj) > 7 and relativeObj[7].PersonImage!=None):
